...
trap {
    ...
    Add-Content -LiteralPath $failureLog -Value "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] 完整流程自检：$technicalMessage" -Encoding UTF8
    $customerMessage = if ($technicalMessage -match '[\u4e00-\u9fff]') { $technicalMessage } else { '电脑返回了技术错误，请联系实施人员处理。' }
    Write-Host ''
    Write-Host "完整流程自检未通过：$customerMessage" -ForegroundColor Red
    Write-Host '请把项目根目录的“部署故障详情.txt”发给实施人员。客户原图不会受影响。'
    exit 1
}
