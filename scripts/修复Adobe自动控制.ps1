...
trap {
    ...
    Add-Content -LiteralPath $failureLog -Value "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Adobe 连接修复：$technicalMessage" -Encoding UTF8
    $customerMessage = if ($technicalMessage -match '[\u4e00-\u9fff]') { $technicalMessage } else { '电脑返回了技术错误，请联系实施人员处理。' }
    Write-Host ''
    Write-Host "修复未完成：$customerMessage" -ForegroundColor Red
    Write-Host '没有删除任何 Adobe 文件。请把项目根目录的“部署故障详情.txt”发给实施人员。'
    exit 1
}
